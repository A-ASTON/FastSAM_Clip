...
public:
    ...
    explicit Inference(const std::string &engine_file_path)
    {
        ...
        // 传入的是.engine文件，获取文件大小
        file.seekg(0, std::ios::end);
        auto size = file.tellg();
        file.seekg(0, std::ios::beg);

        char *trtModelStream = new char[size]; // 这里称其为trt模型流 文件，就是engine
        assert(trtModelStream);
        file.read(trtModelStream, size);
        file.close();
        model_name = engine_file_path.substr(engine_file_path.find_last_of('/') + 1, engine_file_path.length()); // 获取模型文件名 FastSAM.engine
        initLibNvInferPlugins(&this->gLogger, ""); // 传入一个日志记录器，初始化TensorRT的推理插件
    ...
    void make_pipe(int cnt, bool warmup = true)
    {
        ...
        if (warmup) // 执行多次空推理，确保CUDA驱动程序和硬件处于高效状态，减少首次推理的延迟
        {
            for (int i = 0; i < cnt; i++)
            {
                for (auto &bindings : this->input_bindings)
