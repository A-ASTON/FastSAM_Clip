...
void FastSam::letterbox(const cv::Mat &image, cv::Mat &out, cv::Size &size)
{
    ...
    float r = std::min(inp_h / height, inp_w / width); // ratio 
    int padw = std::round(width * r); // padw, 
    int padh = std::round(height * r); // padh, if inp_h > height, ratio > 1, padh > height
...
void FastSam::postprocess(std::vector<Object> &objs)
{
    ...
    int mask_number = this->output_bindings[5].dims.d[1] - class_number - 4; // 32
    int candidates = this->output_bindings[5].dims.d[2]; // 候选数 6300
    ...
    auto *output = static_cast<float *>(this->host_ptrs[5]); // 输出
...
void FastSam::draw_objects(const Mat &image, Mat &res, const vector<Object> &objs)
{
    ...
    for (auto &obj : objs)
    {
        ...
        res(obj.rect).setTo(mask_color, obj.boxMask);
    }
