...
bool app_params_yaml(std::string yaml_filename, app_params & params) {
    // 通过yaml构造app_params
    YAML::Node config = YAML::LoadFile(yaml_filename);
    if (config["model"]) {
        params.model = config["model"].as<std::string>();
    } else {
        printf("please specify a model path");
        return false;
    }
    if (config["threads"]) {
        params.n_threads = config["threads"].as<int>();
    }
    if (config["image"]) {
        params.image_paths.push_back(config["image"].as<std::string>());
    }
    if (config["verbose"]) {
