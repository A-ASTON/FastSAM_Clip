...
AxisAlignedBoundingBox Camera::getViewAABB(const Transform& T_L_C,
  ...
  Eigen::Matrix<float, 8, 3> corners_C = getViewCorners(min_depth, max_depth); // 8个边界点的坐标

  Vector3f aabb_min, aabb_max;
  aabb_min.setConstant(std::numeric_limits<float>::max()); // 初始化
  aabb_max.setConstant(std::numeric_limits<float>::lowest());

  // Transform it into the layer coordinate frame.
  for (size_t corner_idx = 0; corner_idx < corners_C.rows(); corner_idx++) { // 遍历每个边界点
    const Vector3f& corner_C = corners_C.row(corner_idx);
    Vector3f corner_L = T_L_C * corner_C; // 根据T_L_C变换，将其转换到Layer坐标系下的坐标
    for (int i = 0; i < 3; i++) {
  ...
  return AxisAlignedBoundingBox(aabb_min, aabb_max); // 返回AABB，只需要知道最左下角和最右上角的点即可知道整个框，因为是AABB~
}
...
Eigen::Matrix<float, 8, 3> Camera::getViewCorners(const float min_depth,
  ...
  corners_C.row(0) = min_depth * ray_2_C; // 最小深度乘以归一化坐标等于最小深度处的真实坐标
...
Frustum::Frustum(const Camera& camera, const Transform& T_L_C, float min_depth,
                 float max_depth) {
  Eigen::Matrix<float, 8, 3> corners_C =
      camera.getViewCorners(min_depth, max_depth); // 8个边界点的坐标
  computeBoundingPlanes(corners_C, T_L_C); // 计算
}
