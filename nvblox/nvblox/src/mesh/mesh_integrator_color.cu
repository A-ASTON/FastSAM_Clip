...
namespace nvblox {
// 底层调用了colorMeshGPU，输入了ColorLayer和block_indices以及meshLayer
...
__global__ void colorMeshBlockByClosestColorVoxel(
  ...
  // Block
  const ColorBlock* color_block_ptr = color_blocks[blockIdx.x]; // 直接用了线程块ID，因为刚好线程块对应block，线程对应voxel
  ...
  for (int i = threadIdx.x; i < cuda_mesh_block.vertices_size;
       i += blockDim.x) { //i用了当前线程id，然后通过+blockDim.x加上block大小
