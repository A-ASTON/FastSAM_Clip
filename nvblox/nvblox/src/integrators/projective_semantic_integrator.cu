...
ProjectiveSemanticIntegrator::ProjectiveSemanticIntegrator(SemanticConfig& semantic_config) 
    : semantic_config_(semantic_config) {
  semantic_config_.semantic_color_tool_ =
       std::make_unique<SemanticLabel2Color>(semantic_config_.filename);
  sphere_tracer_.maximum_ray_length_m(max_integration_distance_m_);
  checkCudaErrors(cudaStreamCreate(&integration_stream_));
}
...
// 这是mapper调用的函数，可以视作projective_semantic_integrator的入口函数吧
// 预处理获取semantic_labels
void ProjectiveSemanticIntegrator::integrateFrame(
  ...
  std::vector<Index3D> block_indices = view_calculator_.getBlocksInViewPlanes(
      ...
      max_integration_distance_m_ + truncation_distance_m); // 获取视锥体中的block的index
  ...
  // 也就是接收图像->计算视锥体内的blocks->获取在截断距离band内的blocks->更新blocks
  ...
  allocateBlocksWhereRequired(block_indices, semantic_layer); // 在semantic_layer分配需要的blocks，实际上在GPU上也分配了
  ...
  updateBlocks(block_indices, semantic_frame, synthetic_depth_image_, T_L_C,
               camera, truncation_distance_m, semantic_layer); // 在GPU上更新Blocks
...
float ProjectiveSemanticIntegrator::truncation_distance_vox() const {
  return truncation_distance_vox_;
}

float ProjectiveSemanticIntegrator::max_weight() const { return max_weight_; }

float ProjectiveSemanticIntegrator::max_integration_distance_m() const {
  return max_integration_distance_m_;
}

void ProjectiveSemanticIntegrator::truncation_distance_vox(
    float truncation_distance_vox) {
  CHECK_GT(truncation_distance_vox, 0.0f);
  truncation_distance_vox_ = truncation_distance_vox;
}

void ProjectiveSemanticIntegrator::max_weight(float max_weight) {
  CHECK_GT(max_weight, 0.0f);
  max_weight_ = max_weight;
}

void ProjectiveSemanticIntegrator::max_integration_distance_m(
...
float ProjectiveSemanticIntegrator::get_truncation_distance_m(
    float voxel_size) const {
  return truncation_distance_vox_ * voxel_size;
}

WeightingFunctionType ProjectiveSemanticIntegrator::weighting_function_type()
    const {
  return weighting_function_type_;
}

void ProjectiveSemanticIntegrator::weighting_function_type(
    WeightingFunctionType weighting_function_type) {
  weighting_function_type_ = weighting_function_type;
}

const ViewCalculator& ProjectiveSemanticIntegrator::view_calculator() const {
  return view_calculator_;
}

/// Returns the object used to calculate the blocks in camera views.
ViewCalculator& ProjectiveSemanticIntegrator::view_calculator() {
  return view_calculator_;
}

__device__ inline void updateSemanticVoxelProbabilities(
    const SemanticProbabilities& measurement_frequencies,
    SemanticProbabilities* semantic_prior_probability) {
  // DCHECK(semantic_prior_probability != nullptr);
  // DCHECK_EQ(semantic_prior_probability->size(), kTotalNumberOfLabels);
  // DCHECK_LE((*semantic_prior_probability)[0], 0.0);// 未知标签的先验概率<=0
  // DCHECK(std::isfinite((*semantic_prior_probability)[0]));// 未知标签的先验概率有限
  // DCHECK(!semantic_prior_probability->hasNaN());// 没有NaN
  // DCHECK_EQ(measurement_frequencies.size(), kTotalNumberOfLabels);// 数目和总标签数一致
  // DCHECK_GE(measurement_frequencies.sum(), 1.0) // 至少有一个观测
  ...
  semantic_log_likelihood_ = semantic_log_likelihood_.Constant(std::log(0.1));
  semantic_log_likelihood_.diagonal() =
      semantic_log_likelihood_.diagonal().Constant(std::log(0.9));
  semantic_log_likelihood_.col(kUnknownSemanticLabelId).setZero();
  *semantic_prior_probability +=
      semantic_log_likelihood_ * measurement_frequencies; // 后验 = 先验 +  对数似然 * 测量频率（所谓测量频率，就是对应的标签为1，其他为0）
  // DCHECK(!semantic_prior_probability->hasNaN());// 一次性更新所有，每个标签独立计算
...
__device__ inline void calculateMaximumLikelihoodLabel(
  ...
  semantic_posterior.maxCoeff(semantic_label);// 返回最大值的下标，极大似然
}

__device__ inline Color blendTwoSemanticColors(SemanticVoxel* voxel_ptr,
                                       const Color& first_color,
                                       float first_weight,
                                       const Color& second_color,
                                       float second_weight,
                                       SemanticProbabilities measurement_frequencies,
                                       HashableColor* color_map,
                                       int color_map_size) {
  // 改为label融合，然后根据label再获取颜色

  // updateSemanticVoxelProbabilities(measurement_frequencies,
  //                                 &voxel_ptr->semantic_priors);
                          
  // // Get MLE semantic label. 获取极大似然估计 语义标签，semantic_voxel->semantic_priors中最大后验概率的标签则为该体素的标签
  // calculateMaximumLikelihoodLabel(voxel_ptr->semantic_priors,
  ...
  if (!getColorFromSemanticLabel(color_map, 
          color_map_size,
          voxel_ptr->semantic_label,
          &new_color)) {
    // printf("get color from label failed, label:%d", voxel_ptr->semantic_label);
    return Color();
  }

  return Color(new_color.r, new_color.g, new_color.b);
}
...
__device__ inline bool updateVoxel(const Color color_measured,
  ...
  // Read CURRENT voxel values (from global GPU memory)
  const Color voxel_color_current = voxel_ptr->color;
  const float voxel_weight_current = voxel_ptr->weight; // weight在kimera semantic voxel中用不到
  // Fuse
  const float measurement_weight = weighting_function(
      measured_depth_m, voxel_depth_m, truncation_distance_m);
  ...
  const Color fused_color =
      blendTwoSemanticColors(voxel_ptr, voxel_color_current, voxel_weight_current, color_measured,
                     measurement_weight, semantic_label_frequencies, color_map, color_map_size);
  const float weight =
      fmin(measurement_weight + voxel_weight_current, max_weight);
  // Write NEW voxel values (to global GPU memory)
  voxel_ptr->color = fused_color;
  // printf("update voxel and set color:%d,%d,%d\n", voxel_ptr->color.r, voxel_ptr->color.g, voxel_ptr->color.b);
  voxel_ptr->weight = weight;
  voxel_ptr->semantic_label = semantic_label;
  return true;
}
...
__global__ void integrateBlocks(
    ...
    HashableColor* color_map,
    int color_map_size) {
  // SemanticLabelToColorDeviceMap semantic_label_to_color_map 出现了CUDA error = 719
  ...
  // 获取像素平面上的投影像素坐标，体素深度，体素中心点，根据block和thread获取的，即可分配到每个voxel
  ...
  // 通过插值获得的像素的颜色值
  // 问题1：语义分割图的颜色对应于标签（一一对应的，如果插值获取的颜色值是处于边界的，那么得到的颜色值将没有对应的标签！如何处理？）
  // 最近邻插值算法，缺点在哪，有问题吗？有能改进的方法吗
  Color image_value;
  if (!interpolation::interpolate2DClosest<
            Color, interpolation::checkers::ColorPixelAlphaGreaterThanZero>(
            semantic_image, u_px, semantic_rows, semantic_cols, &image_value)) {
    return;
  }


  SemanticLabel semantic_label; //核函数中不能定义指针？ 可以才对，SemanticLabel* semantic_label;导致了error = 719
  // 问题，获取失败还是插值结果有问题？
  if (!getSemanticLabelFromColor(
          ...
          &semantic_label)) {
    printf("read label failed!\n");
    return;
  }
  // 能够获取到label，问题是是updateVoxel出错吗
  // printf("pixel:(%f, %f),  color:(%d, %d, %d), label:%d\n", u_px[0], u_px[1], image_value.r, image_value.g, image_value.b,semantic_label);
  ...
  updateVoxel(image_value, surface_depth_m, voxel_depth_m, max_weight,
              truncation_distance_m, weighting_function, voxel_ptr, semantic_label, color_map, color_map_size);
  // updateVoxel(image_value, surface_depth_m, voxel_depth_m, max_weight,
...
void ProjectiveSemanticIntegrator::updateBlocks(
  ...
  const int depth_subsampling_factor = semantic_frame.rows() / depth_frame.rows(); // 深度下采样系数？RGB / 深度
  ...
  integrateBlocks<<<num_thread_blocks, kThreadsPerBlock, 0, integration_stream_>>>(
      ...
      semantic_config_.semantic_color_tool_->color_map_device_,
      semantic_config_.semantic_color_tool_->color_map_size_);
  // clang-format on
  cudaStreamSynchronize(integration_stream_);
  checkCudaErrors(cudaPeekAtLastError());
}

__global__ void checkSemanticBlocksInTruncationBand(
...
ProjectiveSemanticIntegrator::reduceBlocksToThoseInTruncationBand(
  ...
  // clang-format off
  checkSemanticBlocksInTruncationBand<<<num_thread_blocks, kThreadsPerBlock, 0, integration_stream_>>>(
