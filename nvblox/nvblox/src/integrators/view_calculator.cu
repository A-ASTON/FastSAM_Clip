...
__global__ void combinedBlockIndicesInImageKernel(
  ...
  int pixel_col = ray_idx_col * raycast_subsampling_factor; // 得到当前线程处理的像素坐标
  ...
  // Look up the pixel we care about.
  float depth = image::access<float>(pixel_row, pixel_col, cols, image); // 获取深度值
  if (depth <= 0.0f) {
    return;
  }
  if (max_integration_distance_m > 0.0f && depth > max_integration_distance_m) {
    depth = max_integration_distance_m; // 深度值截断在最大integration距离
  }

  // Ok now project this thing into space.
  Vector3f p_C = (depth + max_integration_distance_behind_surface_m) *
                 camera.vectorFromPixelIndices(Index2D(pixel_col, pixel_row)); //得到相机坐标系下的点，最远点，即截断距离处的点
  Vector3f p_L = T_L_C * p_C; // 转换到layer坐标系下

  // Now we have the position of the thing in space. Now we need the block
  // index.
  Index3D block_index = getBlockIndexFromPositionInLayer(block_size, p_L); // 获取block_index
  setIndexUpdated(block_index, aabb_min, aabb_size, aabb_updated); // 标记为待更新
  ...
  while (raycaster.nextRayIndex(&ray_index)) {
    setIndexUpdated(ray_index, aabb_min, aabb_size, aabb_updated); // 执行光线传播，并将光线经过的block标记为待更新
  }
}

template <typename SensorType> // getBlocksInImageViewRaycastTemplate模板类，类似于integrateFrameTemplate一样，供给两个版本的（lidar和camera）的实际函数调用
std::vector<Index3D> ViewCalculator::getBlocksInImageViewRaycastTemplate(
  ...
  // Aight so first we have to get the AABB of this guy. (0 到 最大截断距离)的AABB，最小深度为0，最大深度为最大截断距离（Layer坐标系下）
  const AxisAlignedBoundingBox aabb_L =
      camera.getViewAABB(T_L_C, 0.0f, max_integration_distance_m);

  // Get the min index and the max index. 获取最小和最大的Block索引，即左下角block和右上角block，由此便可以确认哪些block需要更新，AABB！以minblock和maxblock为左下顶点和右上顶点的平行坐标轴的立方体~
  ...
  const Index3D aabb_size = max_index - min_index + Index3D::Ones(); // 加上1，即包含了max_index本身
  const size_t aabb_linear_size = aabb_size.x() * aabb_size.y() * aabb_size.z(); // 线性尺寸
  ...
  if (aabb_linear_size > aabb_device_buffer_.capacity()) { // 扩容
  ...
  checkCudaErrors(cudaMemsetAsync(aabb_device_buffer_.data(), 0,
                                  sizeof(bool) * aabb_linear_size)); // 线性的3D grid mask，用于存放布尔值表示一个block是否在视场内
  aabb_device_buffer_.resize(aabb_linear_size);
  aabb_host_buffer_.resize(aabb_linear_size);

  setup_timer.Stop();
  // 以上的逻辑就是：通过T_L_C获取从0到最大截断距离内的，layer坐标系下的AABB包围盒，利用该包围盒获取对应的block索引，
  // 并构建一个aabb_linear_size的aabb_*_buffer用于接下来记录哪些block在raycast过程中被光线穿过，从而需要更新！

  // Raycast 计算涉及更新的block，通过aabb_device_buffer存放
  ...
  convertAabbUpdatedToVector<std::vector<Index3D>>(
      ...
      &output_vector); // 将涉及更新的线性索引，根据aabb包围盒转换到layer坐标系下的索引
  output_timer.Stop();
...
void ViewCalculator::getBlocksByRaycastingPixels(
  ...
  const int num_subsampled_rows =
      std::ceil(static_cast<float>(depth_frame.rows() + 1) /
                static_cast<float>(raycast_subsampling_factor_)); // 下采样之后的深度图行数即height
  const int num_subsampled_cols =
      std::ceil(static_cast<float>(depth_frame.cols() + 1) /
                static_cast<float>(raycast_subsampling_factor_)); // 下采样之后的深度图列数即width
  // We'll do warps of 32x32 pixels in the image. This is 1024 threads which is
  // in the recommended 512-1024 range.
  constexpr int kThreadDim = 16;
  const int rounded_rows = static_cast<int>(
      std::ceil(num_subsampled_rows / static_cast<float>(kThreadDim))); // height / 16 
  const int rounded_cols = static_cast<int>(
      std::ceil(num_subsampled_cols / static_cast<float>(kThreadDim)));
  dim3 block_dim(rounded_rows, rounded_cols); // 一个线程块，包含了height / 16, width / 16块线程
  dim3 thread_dim(kThreadDim, kThreadDim); // 每个线程块包含了16 * 16个线程 256个线程
  // 也就是刚好每个线程处理一个像素
...
std::vector<Index3D> ViewCalculator::getBlocksInViewPlanes(
  ...
  const Frustum frustum =
      camera.getViewFrustum(T_L_C, kMinDistance, max_distance); // 根据相机位姿和min max距离获取视椎体

  // Coarse bound: AABB
  const AxisAlignedBoundingBox aabb_L = frustum.getAABB();
  // aabb框中的block indices
