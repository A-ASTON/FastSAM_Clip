...
struct UpdateTsdfVoxelFunctor {
  ...
  // Vector3f p_voxel_C, float depth, TsdfVoxel* voxel_ptr 定义了操作，就是进行一个tsdf更新
  __device__ bool operator()(const float surface_depth_measured,
    ...
    const float voxel_to_surface_distance =
        surface_depth_measured - voxel_depth_m; // 计算体素到表面的距离 sdf

    // If we're behind the negative truncation distance, just continue. 在负的截止范围外，跳过
    ...
    voxel_ptr->distance = fused_distance; // 写入新的sdf和权重，通过voxel指针实现的
...
unified_ptr<UpdateTsdfVoxelFunctor> // 表明了函子是在统一内存上的
ProjectiveTsdfIntegrator::getTsdfUpdateFunctorOnDevice(float voxel_size) {
  ...
  update_functor_host_ptr_->truncation_distance_m_ =
      get_truncation_distance_m(voxel_size); // 通过体素大小获取截断距离(单位m)并设置
  update_functor_host_ptr_->weighting_function_ =
      WeightingFunction(weighting_function_type_); // 获取并设置权重函数
  // Transfer to the device
  return update_functor_host_ptr_.clone(MemoryType::kDevice); // 返回设备上的更新函子指针
}
...
void ProjectiveTsdfIntegrator::integrateFrame(
  ...
  // Get the update functor on the device 在设备上获取更新函子
