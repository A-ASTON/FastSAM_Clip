...
Mapper::Mapper(float voxel_size_m, MemoryType memory_type,
  ...
  layers_ = LayerCake::create<TsdfLayer, SemanticLayer, ColorLayer, OccupancyLayer, EsdfLayer,
                              MeshLayer>(voxel_size_m_, memory_type);
}

Mapper::Mapper(float voxel_size_m, ProjectiveSemanticIntegrator::SemanticConfig& semantic_config, MemoryType memory_type,
               ProjectiveLayerType projective_layer_type)
    : voxel_size_m_(voxel_size_m),
      memory_type_(memory_type),
      projective_layer_type_(projective_layer_type), semantic_integrator_(semantic_config) {
  layers_ = LayerCake::create<TsdfLayer, SemanticLayer, ColorLayer, OccupancyLayer, EsdfLayer,
                              MeshLayer>(voxel_size_m_, memory_type);
}
...
void Mapper::integrateSemantic(const ColorImage& semantic_frame, const Transform& T_L_C,
                      const Camera& camera) {
...
void Mapper::integrateDepth(const DepthImage& depth_frame,
  ...
  // Call the integrator.
  std::vector<Index3D> updated_blocks; // 指出涉及更新的block，在执行integrate后会返回这些值，是通过view_calculator_.getBlocksInImageViewRaycast计算得到的
  if (projective_layer_type_ == ProjectiveLayerType::kTsdf) {
    tsdf_integrator_.integrateFrame(depth_frame, T_L_C, camera,
                                    ...
                                    &updated_blocks); // 执行tsdf integrate
    // The mesh is only updated for the tsdf projective layer
    mesh_blocks_to_update_.insert(updated_blocks.begin(), updated_blocks.end()); // 需要更新mesh的blocks
  } else if (projective_layer_type_ == ProjectiveLayerType::kOccupancy) { // 如果是occupancy模式，就调用occupancy_integrator_的函数
    occupancy_integrator_.integrateFrame(depth_frame, T_L_C, camera,
                                         layers_.getPtr<OccupancyLayer>(),
                                         &updated_blocks);
  }
  // Update all the relevant queues.
  esdf_blocks_to_update_.insert(updated_blocks.begin(), updated_blocks.end()); // 需要更新esdf的blocks
}

// 对lidar深度图进行integrate
void Mapper::integrateLidarDepth(const DepthImage& depth_frame,
  ...
  // Update all the relevant queues.
  esdf_blocks_to_update_.insert(updated_blocks.begin(), updated_blocks.end()); // 插入需要更新的block的Index3D
}

// 将单次RGB信息进行integrate，传入位姿，相机模型，RGB图像，调用integrateFrame
...
void Mapper::decayOccupancy() {
  // The esdf of all blocks has to be updated after decay 所有block的esdf都需要在decay后再进行更新
...
std::vector<Index3D> Mapper::updateMesh() {
  ...
  //                            mesh_blocks_to_update_vector,
  //                            layers_.getPtr<MeshLayer>());
  mesh_integrator_.semanticMesh(layers_.get<SemanticLayer>(),
                             mesh_blocks_to_update_vector,
                             layers_.getPtr<MeshLayer>());
