...
void Timing::Print(std::ostream& out) {
  ...
  for (typename map_t::value_type t : tagMap) {
    ...
    out << GetNumSamples(i) << "\t"; // 总样本数
    if (GetNumSamples(i) > 0) {
      out << SecondsToTimeString(GetTotalSeconds(i)) << "\t"; // 总时间
      double meansec = GetMeanSeconds(i); // 均值
      double stddev = sqrt(GetVarianceSeconds(i));
      out << "(" << SecondsToTimeString(meansec) << " +- ";
      out << SecondsToTimeString(stddev) << ")\t"; // 标准差

      double minsec = GetMinSeconds(i); // 最短时间
      double maxsec = GetMaxSeconds(i); // 最长时间
