...
// 这里包含了gpu_layer_view它对GPULayerView进行了声明
// 以及gpu_hash_interface.cuh，它对GPUHashImpl进行了声明，GPULayerView依赖于它
namespace nvblox {
...
void GPULayerView<BlockType>::reset(LayerType* layer_ptr) {
  ...
  if (!gpu_hash_ptr_) {
    ...
    // 调用了GPUHashImpl的构造函数，传入了map所需大小，block数 * 2
  }
  ...
  const float current_load_factor =
      ...
      static_cast<float>(gpu_hash_ptr_->max_num_blocks_); // 当前负载因子
  if (current_load_factor > max_load_factor_) { // 如果大于0.5,则需要扩容
  ...
  // 插入到map中，unordered_map需要const key，但是IndexBlockPair不是常量key，需要上面的转换
