...
class Mapper : public MapperBase {
 public:
  ...
  // for semantic
  Mapper(
      float voxel_size_m, ProjectiveSemanticIntegrator::SemanticConfig& semantic_config, MemoryType memory_type = MemoryType::kDevice, 
      ProjectiveLayerType projective_layer_type = ProjectiveLayerType::kTsdf);
  ...
  ///@param semantic_frame semantic image to integrate.
  ...
  void integrateSemantic(const ColorImage& semantic_frame, const Transform& T_L_C,
                      const Camera& camera);
