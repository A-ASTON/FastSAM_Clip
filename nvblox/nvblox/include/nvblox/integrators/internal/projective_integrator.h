...
class ProjectiveIntegrator {
 public:
  ...
  /// A parameter getter 截断距离，以体素为单位
  ...
  /// distance for.
  /// @return The truncation distance 获取度量截断距离(m)
  float get_truncation_distance_m(float voxel_size) const;

  /// A parameter getter  最大更新距离，超过该距离的体素不会被更新
  ...
  /// A parameter setter 截断距离，以体素为单位
  ...
  /// Returns the object used to calculate the blocks in camera views. 返回一个用于计算相机视图内可见块的ViewCalculator对象
 ...
 protected:
  ...
  // voxels within these blocks on the GPU. 在GPU上对指定块内所有体素执行TSDF更新
  template <typename UpdateFunctor>
  ...
  // Frustum calculation. 表示在const 成员函数中被修改
