...
// CAMERA 底层的integrateBlock核函数 更新所有block
...
__global__ void integrateBlocksKernel(
  ...
  // in the block (z-major). 为当前线程指定计算的voxel
  VoxelType* voxel_ptr = &(block_device_ptrs[blockIdx.x] // 不同线程块处理不同的block
                               ->voxels[threadIdx.z][threadIdx.y][threadIdx.x]); // 不同的线程处理不同的voxel

  // Update the voxel using the update rule for this layer type
  (*op)(image_value, voxel_depth_m, voxel_ptr); // 使用op更新voxel，传入surface_depth_measured和voxel_depth_m,以及对应的体素指针，实现体素的tsdf更新
}
...
void ProjectiveIntegrator<VoxelType>::integrateFrameTemplate(
  ...
  const float max_integration_distance_behind_surface_m = // 最大截断距离，距离表面多远就不参与更新了
      truncation_distance_vox_ * layer->voxel_size();
  const std::vector<Index3D> block_indices = // 获取涉及更新的block_indices Eigen::Vector3i
  ...
  allocateBlocksWhereRequired(block_indices, layer); // CPU函数，分配block
...
// Camera integrateFrame的实现，实际使用时需要指明VoxelType并传入op更新函子
template <typename VoxelType>
...
// Camera integrate block的主机函数，调用核函数integrateBlocksKernel
template <typename VoxelType>
template <typename UpdateFunctor>
...
void ProjectiveIntegrator<VoxelType>::integrateBlocksTemplate(
  ...
  // Expand the buffers when needed 如果block_indices_device_的容量不够，则进行扩容
  // 场景过大的时候就会出现扩容，或者T_L_C飘了
  ...
  // Transfer to the device 通过pinned memory，可以方便地实现数据从host到device的拷贝转移
