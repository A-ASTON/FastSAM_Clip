...
class MeshIntegrator {
 public:
  ...
  /// Semantic mesh layer
  /// 实际上就是用semantic layer代替color layer给mesh附上颜色而已
  void semanticMesh(const SemanticLayer& semantic_layer, MeshLayer* mesh_layer);
  void semanticMesh(const SemanticLayer& semantic_layer,
                 const std::vector<Index3D>& block_indices,
                 MeshLayer* mesh_layer);
  void semanticMeshGPU(const SemanticLayer& semantic_layer, MeshLayer* mesh_layer);
  void semanticMeshGPU(const SemanticLayer& semantic_layer,
                    const std::vector<Index3D>& block_indices,
                    MeshLayer* mesh_layer);
  void semanticMeshCPU(const SemanticLayer& semantic_layer, MeshLayer* mesh_layer);
  void semanticMeshCPU(const SemanticLayer& semantic_layer,
                    const std::vector<Index3D>& block_indices,
                    MeshLayer* mesh_layer);
